...
void play()
{
  ...
  for(int i=0; i<16; i++) for(int j=0; j<8; j++) for(int k=0; k<8; k++) if(piece[1][i].exists && piece[1][i].possible(j,k))
  {
    ...
    else
    {
      ...
      if(temp != null && temp.score == optimal.score && random(8) < 1) optimal = temp;
      else if(temp != null && temp.score > optimal.score) optimal = temp;
    }
  }
  
  for(int i=0; i<16; i++) if(piece[1][i].exists && piece[1][i].possible(piece[0][15].x,piece[0][15].y))
  {
    piece[1][i].move(piece[0][15].x,piece[0][15].y);
    optimal = null;
    break;
  }
...
Move alphac(int sx, int sy, int tx, int ty, int osx, int osy, int otx, int oty, int d, Move min)
{
  ...
  else
  {
    for(int i=0; i<16; i++) for(int j=0; j<8; j++) for(int k=0; k<8; k++) if(piece[0][i].exists && piece[0][i].possible(j,k))
    {
      ...
      else
      {
        ...
        if(temp != null && temp.score == worst.score && random(8) < 1) worst = temp;
        else if(temp != null && temp.score < worst.score) worst = temp;
      }
      
      if(worst != null && min != null && worst.score < min.score)
      {
  ...
  if(worst == null) worst = new Move(osx,osy,otx,oty,evaluate());
...
Move betac(int sx, int sy, int tx, int ty, int osx, int osy, int otx, int oty, int d, Move max)
{
  ...
  else
  {
    for(int i=0; i<16; i++) for(int j=0; j<8; j++) for(int k=0; k<8; k++) if(piece[1][i].exists && piece[1][i].possible(j,k))
    {
      ...
      else
      {
        ...
        if(temp != null && temp.score == best.score && random(8) < 1) best = temp;
        else if(temp != null && temp.score > best.score) best = temp;
      }
      
      if(best != null && max != null && best.score > max.score)
      {
        square[tx][ty].piece.move(sx,sy);
  ...
  if(best == null) best = new Move(osx,osy,otx,oty,evaluate());
